package cn.dreamfruits.yaoguo.module.OEMPush.pushnative;
...
import cn.dreamfruits.yaoguo.module.OEMPush.OEMPushSetting;
...
import cn.dreamfruits.yaoguo.module.OEMPush.notification.ParseNotification;
...
import cn.dreamfruits.yaoguo.module.OEMPush.utils.TUIOfflinePushParamBean;
import cn.dreamfruits.yaoguo.module.OEMPush.utils.TUIOfflinePushUtils;
...
public class TUIOfflinePushManager {
    public static final String TAG = ">>>> push " + TUIOfflinePushManager.class.getSimpleName();
     ...
     * @note If you do not use the interface provided by TUILogin, you need to manually call the interface after completing the login operation to enable the push
     * service
     */
    public void registerPush(Context context) {
        if (!mAutoRegisterPush) {
            LogUtils.e(TAG, "you are calling api: registerPush(String json, Context context)");
            return;
        }
        initConfig(context);
        initPush(context);
    ...
    public void registerPush(String json, Context context) {
        ...
        LogUtils.e(TAG, "消息推送 success = " + success);
        if (success) {
            LogUtils.e(TAG, "消息推送 success");
            initPush(context);
        } else {
            LogUtils.e(TAG, "parseJson2TUIOfflinePushParamBean failed");
        }
    }
     ...
     * @note If you do not use the interface provided by TUILogin, you need to manually call this interface after completing the logout operation to close the push
     * service
     */
    ...
    private void initPush(Context context) {
        ...
        pushSetting.setPushCallback(new PushCallback() {
            ...
            public void onTokenCallback(String token) {
                ...
                LogUtils.e(TAG, "消息推送注册 onTokenCallback token = " + token);
            ...
            public void onTokenErrorCallBack(TUIOfflinePushErrorBean errorBean) {
                if (errorBean != null) {
                    LogUtils.e(TAG, "onTokenErrorCallBack code = " + errorBean.getErrorCode() + ", code des = " + errorBean.getErrorDescription());
                }
            }
        });
    ...
    public void parseNotificationAndSendBroadCast() {
        ...
        if (TextUtils.isEmpty(ext)) {
            LogUtils.e(TAG, "ext is null");
        }
        LogUtils.d(TAG, "parseNotificationAndSendBroadCast ext = " + ext);
        ...
        LocalBroadcastManager.getInstance(TUIOfflinePushService.appContext).sendBroadcast(intent);
        LogUtils.d(TAG, "parseNotificationAndSendBroadCast sendBroadcast = " + mNotificationIntent);*/
    }
    ...
    void setPushTokenToTIM(String token) {
        ...
        if (mBussinessId == 0) {
            ...
            LogUtils.d(TAG, "setPushTokenToTIM brand is" + brand);
            if (isInternationalFlavor) {
                LogUtils.i(TAG, "flavor international");
                switch (brand) {
        ...
        LogUtils.d(TAG, "setOfflinePushConfig businessID = " + mBussinessId + " pushToken = " + pushToken);

        V2TIMManager.getOfflinePushManager().setOfflinePushConfig(v2TIMOfflinePushConfig, new V2TIMCallback() {
            @Override
            public void onError(int code, String desc) {
                LogUtils.d(TAG, "setOfflinePushToken err code = " + code + " desc = " + ErrorMessageConverter.convertIMError(code, desc));
            }

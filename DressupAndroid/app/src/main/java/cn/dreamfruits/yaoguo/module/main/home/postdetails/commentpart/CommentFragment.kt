...
class CommentFragment(
    ...
    var feedId: Long,
    var highlightId: Long,//高亮id
    var isComment: Boolean = false,//是否弹出评论
    var messageIds: String,//消息页过来需要的id
    var messageCommentIds: String//消息页面过来的id
) : Fragment() {
    ...
    override fun onCreateView(
        ...
        mView = inflater.inflate(R.layout.comment_pop, container, false)
    ...
    private var commentDefaultLayout: View? = null//缺省
    ...
    //弹窗独有
    private var popCloseImgView: View? = null
    private var popReplyTv: TextView? = null
...
//    Singleton.bgAlpha(mActivity, 0.9f)

// 创建 Handler 对象
        handler = Handler(Looper.getMainLooper())

        //控件相关
        commentEnd = mView!!.findViewById(R.id.comment_end)//末尾标记
        topView = mView!!.findViewById(R.id.top_view)//末尾标记

        commentCountTv = mView!!.findViewById(R.id.comment_count_tv)//评论数


        mRecyclerview = mView!!.findViewById(R.id.comment_recyclerview) //评论集合
        commentDefaultLayout = mView!!.findViewById(R.id.comment_default_layout) //评论缺省
        // TODO: 作者的id   这里直接用-1
        commentListAdapter = CommentListAdapter(-1,mActivity, mFirstCommentList!!)//评论适配器
        ...
        var popReLayout = mView!!.findViewById<View>(R.id.pop_re_layout)//弹窗布局
        //点击关闭
        popCloseImgView = mView!!.findViewById(R.id.pop_close)
        popCloseImgView!!.setOnClickListener {//隐藏弹出框

            topView!!.setBackgroundColor(Color.parseColor("#ffffffff"))
            topView!!.visibility = View.INVISIBLE
            val anim: Animation = AnimationUtils.loadAnimation(mActivity, R.anim.anim_bottom_out)
            popReLayout!!.startAnimation(anim)
            popReLayout.visibility = View.GONE
...
//        LocalBroadcastManager.getInstance(mActivity).sendBroadcast(intent)
        ...
        timer.schedule(object : TimerTask() {
            override fun run() {
                ...
                handler.post {
                    // 在这里更新 UI
                    topView!!.setBackgroundColor(Color.parseColor("#80333333"))
        ...
        //底部输入框
        popReplyTv = mView!!.findViewById(R.id.pop_reply_tv)
        popReplyTv!!.setOnClickListener { //帖子发布评论
         ...
         * 刷新控件
         */
        val refreshLayout: RefreshLayout = mView!!.findViewById(R.id.refreshLayout)
...
//                    if (mCommentTv!=null){
//                        mTempCommentCount += 1
//                        Singleton.setNumRuler(mTempCommentCount, mCommentTv!!, Singleton.DEFAULT_COMMENT)
//                    }

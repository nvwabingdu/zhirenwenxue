...
import cn.dreamfruits.yaoguo.module.main.home.state.HomeRecommendUserListState
...
class FindUserActivity : BaseActivity() {
    ...
    private fun initTop() {
        ...
        findViewById<ImageView>(R.id.img_qr_code).setOnClickListener {
            ...
            intent.putExtra("userId",Singleton.getUserInfo().userId)//这里传递自己的id
            startActivity(intent)
        }
        //扫一扫
    ...
    private fun setRefresh() {
         ...
         * 2.请求结果
         */
        contactsViewModel.homeRecommendUserListState.observe(this) {
            when (it) {
                is HomeRecommendUserListState.Success -> {
                    when (refreshState) {
                        0 -> {//刷新
                            mList!!.clear()//清空集合
                            mList = contactsViewModel.mHomeRecommendUserListBean!!.list.toMutableList()//加载请求后的数据
                        ...
                        1, 2 -> {//加载 预加载
                            mAdapter!!.setData(
                                contactsViewModel.mHomeRecommendUserListBean!!.list.toMutableList(),
                                false
                            )//设置数据 更新适配器
...
//                    initPop()
        ...
        refreshLayout?.setOnRefreshListener { refreshlayout ->
            ...
            /**请求推荐好友*/
            contactsViewModel.getHomeRecommendUserList(10)
        }
...
//        refreshLayout!!.setEnableLoadMore(false)//设置不加载
        ...
        refreshLayout?.setOnLoadMoreListener { refreshlayout ->
            ...
            if (mList!!.size!=0){
                refreshState=1
                getLoadMoreLogic(contactsViewModel.mHomeRecommendUserListBean!!)
            }
        }
    }
     ...
     * 5.加载更多逻辑
     */
    private fun getLoadMoreLogic(bean: SearchUserBean) {
        //用于分页，当上一次接口请求返回hasNext为1时，将上一次返回的lastTime传入  表示有下一页
     if (bean != null && bean!!.hasNext == 1) {
         /**请求推荐好友*/
         contactsViewModel.getHomeRecommendUserList(10)
    ...
    private fun requestContactsPermission() {
        ...
        PermissionX.init(this).permissions(
            ...
            override fun onForwardToSettings(scope: ForwardScope, deniedList: MutableList<String>) {
                Singleton.centerToast(this@FindUserActivity, "您永久拒绝了该权限，请到手机设置更改……")
            }
        })
            .request(object : RequestCallback {
                override fun onResult(
                    ...
                    if (allGranted) {
                        ...
                        // 有权限被拒绝
                        Singleton.centerToast(this@FindUserActivity, "您拒绝了如下权限：$deniedList")
                    }
                }
            }
        )

package cn.dreamfruits.selector.preview.play
...
open class VideoPlayer : IVideoPlayer {
        ...
        workHandler = object : Handler(handlerThread.looper) {
            override fun handleMessage(msg: Message) {
                try {
                    ...
                    Log.e(TAG, ",workHandler handleMsg error $msg, $e", )
                }
            }
        }
    }
    ...
    override fun setSurface(surface: Surface) {
        workHandler.post {
            try {
                if (!surface.isValid) {
                    Log.e(TAG, "surface invalid, maybe has been release")
                    return@post
                }
    ...
    override fun play() {
        if (!isDataSourceValid) {
            Log.e(TAG, "dataSource not valid, can not play")
            return
        }
        val msg = Message.obtain(workHandler)
